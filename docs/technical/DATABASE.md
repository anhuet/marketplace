...
| display_name | text | NULL | Publicly visible name chosen by the user. For new users, the application auto-assigns an anonymous `userXXXXXX` (six-digit) value at create time so the column is non-null from the very first request. The legacy NULL state is retained in the schema for backwards compatibility with rows created before this change but is no longer produced by the current code path |
| display_name_lower | text | NULL | Application-maintained lowercase mirror of `display_name`. Written by the application on every update to `display_name`. Uniqueness enforced via a partial unique index (`WHERE display_name_lower IS NOT NULL`). New users always get a non-null value (the lowercased auto-assigned name) |
| needs_display_name_setup | boolean | NOT NULL, DEFAULT false | True when the user must complete ProfileSetup to confirm or change their display name. Set for brand-new users (whose name is the auto-generated `userXXXXXX`) and for existing users whose `display_name` matched their email prefix (PII-leak pattern). Cleared by the application once the user saves a name through ProfileSetup |
...
**Notes**: Hard deletes are used. Listings and conversations should be anonymised or cascade-deleted before user deletion. `average_rating` and `rating_count` are denormalised for read performance — keep them consistent via the review creation transaction. `display_name` is nullable at the schema level but is never NULL for users created by the current code path: `findOrCreateUser` assigns an auto-generated `userXXXXXX` value with a unique six-digit suffix at create time, and `needs_display_name_setup` is set so the mobile app routes the user through ProfileSetup to confirm or change the name. Application code should still handle the NULL case defensively for legacy rows.
...
**NULL semantics**: `display_name` and `display_name_lower` are nullable at the schema level for backwards compatibility with legacy rows. New users created by `findOrCreateUser` always receive a non-null auto-generated `userXXXXXX` value, so NULL should not appear for any row produced by the current code path. The partial unique index (`WHERE display_name_lower IS NOT NULL`) tolerates legacy NULL rows without competing for the unique slot.

**Auto-generated name on signup**: When a new user record is created, the backend generates a candidate `userXXXXXX` (six-digit suffix) name and inserts it directly. Up to 10 candidates are tried against the existing `display_name_lower` values to avoid the partial unique index race; if all collide, a timestamp-suffixed fallback is used. The generated name is intentionally anonymous (no email-prefix PII) but `needs_display_name_setup = true` is set so the mobile app still routes the user through ProfileSetup, where the generated value is prefilled and the user can confirm it or pick a new one.

**Setup flag**: `needs_display_name_setup = true` signals to the mobile app that the user must be routed through ProfileSetup before using the main app. It is set for brand-new users (whose `display_name` is the auto-generated `userXXXXXX`) and for existing users whose `display_name` at migration time was their email local-part (the email-prefix PII leak — these rows are now rewritten to a fresh auto-generated value on the next authenticated request). The application clears this flag once the user saves a name through ProfileSetup.
